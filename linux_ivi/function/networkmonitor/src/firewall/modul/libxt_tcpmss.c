...
tcpmss_print(const void *ip, const struct xt_entry_match *match, int numeric)
{
	...
	sprintf(poolformat," tcpmss match %s", info->invert ? "!" : "");
	strcat(pfirerule->expand,poolformat);
	if (info->mss_min == info->mss_max){
		sprintf(poolformat,"%u", info->mss_min);
		strcat(pfirerule->expand,poolformat);
	}
	else{
		sprintf(poolformat,"%u:%u", info->mss_min, info->mss_max);
		strcat(pfirerule->expand,poolformat);
		}
}
