...
print_port(uint16_t port, int numeric)
{
	...
	if (numeric || (service = port_to_service(port)) == NULL){
		sprintf(poolformat,"%u", port);
		strcat(pfirerule->expand,poolformat);
	}
	else{
		sprintf(poolformat,"%s", service);
		strcat(pfirerule->expand,poolformat);
	}
}
	    ...
	    int invert, int numeric)
{
	...
	if (min != 0 || max != 0xFFFF || invert) {
		sprintf(poolformat,"%s", name);
		strcat(pfirerule->expand,poolformat);
		if (min == max) {
			sprintf(poolformat,":%s", inv);
			strcat(pfirerule->expand,poolformat);
			print_port(min, numeric);
		} else {
			sprintf(poolformat,"s:%s", inv);
			strcat(pfirerule->expand,poolformat);
			print_port(min, numeric);
			strcat(pfirerule->expand,":");
			print_port(max, numeric);
		}
		//strcat(replayinfo.replay[replayinfo.replayindex]->data," ");
	}
}
...
print_option(uint8_t option, int invert, int numeric)
{
	char poolformat[32];
	if (option || invert){
		sprintf(poolformat,"option=%s%u ", invert ? "!" : "", option);
		strcat(pfirerule->expand,poolformat);
	}
}
...
print_tcpf(uint8_t flags)
{
	...
	while (flags) {
		...
		if (have_flag)
			strcat(pfirerule->expand,",");
		sprintf(poolformat,"%s", tcp_flag_names[i].name);
		strcat(pfirerule->expand,poolformat);
		have_flag = 1;
...
print_flags(uint8_t mask, uint8_t cmp, int invert, int numeric)
{
	char poolformat[32];
	if (mask || invert) {
		sprintf(poolformat,"flags:%s", invert ? "!" : "");
		strcat(pfirerule->expand,poolformat);
		if (numeric){
			sprintf(poolformat,"0x%02X/0x%02X ", mask, cmp);
			strcat(pfirerule->expand,poolformat);
		}
		else {
			print_tcpf(mask);
...
tcp_print(const void *ip, const struct xt_entry_match *match, int numeric)
{
	...
	if (tcp->invflags & ~XT_TCP_INV_MASK){
		sprintf(poolformat,"Unknown invflags: 0x%X ",tcp->invflags & ~XT_TCP_INV_MASK);
		strcat(pfirerule->expand,poolformat);
		}
}
