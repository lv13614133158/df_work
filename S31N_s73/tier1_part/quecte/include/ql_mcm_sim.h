...
typedef struct 
{/* when write, if username and phonenum is empty, it means to delete this item specified by file_info.file_access.record_num  */
    char                        username[14];   /**<   username */
    char                        phonenum[24];   /**<   Phone number, it can include '+', such as +13812345678 */
}QL_MCM_SIM_PHONE_RECORD_INFO_T; 

typedef struct 
{
    QL_MCM_SIM_CARD_FILE_INFO_TO_READ_T file_info;      /**< File access information. record_num means which item to read. Normally, file size of 3F007F106F3A is 7000 can store 250 items of 28 bytes size each */
    QL_MCM_SIM_PHONE_RECORD_INFO_T      t_record;       /**< phone info */
...
E_QL_ERROR_CODE_T QL_MCM_SIM_SetPhoneNumber
(
    sim_client_handle_type          h_sim,
    QL_SIM_APP_ID_INFO_T            *pt_info,   ///< [IN] The SIM identifier.
    char                            *phone_num, ///< [IN] phone number
    size_t                          phoneLen    ///< [IN] phone number buffer length
);

/*===========================================================================

  FUNCTION:  QL_MCM_SIM_ReadPhoneBookRecord

===========================================================================*/
/*
    @brief
    Function will get the phone book record as specified by the pt_info.

    record number = 0  -  transparent
    record number > 0  -  record based, means which record to read
...
(
    sim_client_handle_type              h_sim,
    QL_MCM_SIM_PHONE_BOOK_RECORD_INFO_T *pt_info   ///< [OUT] Verify PIN infor
);


/*===========================================================================

  FUNCTION:  QL_MCM_SIM_WritePhoneBookRecord

===========================================================================*/
/*
    @brief
    Function will store the phone book record to the location specified by the pt_info.
    if username and phonenum is empty, it means to delete this item specified by 
    pt_info->file_info.file_access.record_num
