...
static void print_range(const struct nf_nat_ipv4_range *r)
{
	...
	if (r->flags & NF_NAT_RANGE_MAP_IPS) {
		...
		a.s_addr = r->min_ip;
		sprintf(poolformat,"%s", xtables_ipaddr_to_numeric(&a));
	...
	if (r->flags & NF_NAT_RANGE_PROTO_SPECIFIED) {
		...
		sprintf(poolformat,"%hu", ntohs(r->min.tcp.port));
		strcat(pfirerule->expand,poolformat);
		if (r->max.tcp.port != r->min.tcp.port){
			sprintf(poolformat,"-%hu", ntohs(r->max.tcp.port));
			strcat(pfirerule->expand,poolformat);
		}
	}
}
...
static void DNAT_save(const void *ip, const struct xt_entry_target *target)
{
	...
	for (i = 0; i < info->mr.rangesize; i++) {
		strcat(pfirerule->expand,"--to-destination ");
		print_range(&info->mr.range[i]);
		if (info->mr.range[i].flags & NF_NAT_RANGE_PROTO_RANDOM)
			strcat(pfirerule->expand,"--random");
		if (info->mr.range[i].flags & NF_NAT_RANGE_PERSISTENT)
