...
void tbox_get_info(tboxInfo_t *p_tbox_mcu_info) {
	...
	// if (TTier1InfoDftcCreate(&info_case) < 0)
	// 	return;
 
	// if (TTier1InfoDftcInit(&info_case) < 0) {
	// 	log_e("tbox_get_info", "init err\n");
	// 	TTier1InfoDftcDestroy(&info_case);
	// 	return;
	// }
	
	// //获取VIN码
	// len = TTier1InfoDftcVin(&info_case, VIN, sizeof(VIN));
	// if (len > 0) {
	// 	snprintf(p_tbox_mcu_info->VIN, sizeof(p_tbox_mcu_info->VIN), "%s", VIN);
	// 	p_tbox_mcu_info->VIN[len] = '\0';
	// }

	// //获取SN码
	// len = TTier1InfoDftcSN(&info_case, SN, sizeof(SN));
	// if (len > 0) {
	// 	snprintf(p_tbox_mcu_info->ID, sizeof(p_tbox_mcu_info->ID), "%s", SN);
	// 	p_tbox_mcu_info->ID[len] = '\0';
	// }

	// //获取MANUFACTURER
	// len = TTier1InfoDftcManufacturerInfo(&info_case, MANUFACTURER, sizeof(MANUFACTURER));
	// if (len > 0) {
	// 	snprintf(p_tbox_mcu_info->MANUFACTURER, sizeof(p_tbox_mcu_info->MANUFACTURER), "%s", MANUFACTURER);
	// 	p_tbox_mcu_info->MANUFACTURER[len] = '\0';
	// }

	// //获取SYS_VESDION
	// len = TTier1InfoDftcSoftwareVersion(&info_case, SYS_VERSION, sizeof(SYS_VERSION));
	// if (len > 0) {
	// 	snprintf(p_tbox_mcu_info->SYS_VERSION, sizeof(p_tbox_mcu_info->SYS_VERSION), "%s", SYS_VERSION);
	// 	p_tbox_mcu_info->SYS_VERSION[len] = '\0';
	// }

	//  TTier1InfoDftcFini(&info_case);
	//  TTier1InfoDftcDestroy(&info_case);
} 

// void convertAndSetPosition(const TTier1InfoDftcGnssInfo *gnss_info, tbox_gps_info_t *gps_info) {
//     gps_info->latitude = (double)gnss_info->latitude_ / 10000000.0;
//     gps_info->longitude = (double)gnss_info->longitude_ / 10000000.0;
// 	return;
// }
