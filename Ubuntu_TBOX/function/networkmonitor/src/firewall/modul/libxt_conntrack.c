...
print_state(unsigned int statemask)
{
	...
	if (statemask & XT_CONNTRACK_STATE_INVALID) {
		sprintf(poolformat,"%sINVALID", sep);
		strcat(pfirerule->expand,poolformat);
		sep = ",";
	}
	if (statemask & XT_CONNTRACK_STATE_BIT(IP_CT_NEW)) {
		sprintf(poolformat,"%sNEW", sep);
		strcat(pfirerule->expand,poolformat);
		sep = ",";
	}
	if (statemask & XT_CONNTRACK_STATE_BIT(IP_CT_RELATED)) {
		sprintf(poolformat,"%sRELATED", sep);
		strcat(pfirerule->expand,poolformat);
		sep = ",";
	}
	if (statemask & XT_CONNTRACK_STATE_BIT(IP_CT_ESTABLISHED)) {
		sprintf(poolformat,"%sESTABLISHED", sep);
		strcat(pfirerule->expand,poolformat);
		sep = ",";
	}
	if (statemask & XT_CONNTRACK_STATE_UNTRACKED) {
		sprintf(poolformat,"%sUNTRACKED", sep);
		strcat(pfirerule->expand,poolformat);
		sep = ",";
	}
	if (statemask & XT_CONNTRACK_STATE_SNAT) {
		sprintf(poolformat,"%sSNAT", sep);
		strcat(pfirerule->expand,poolformat);
		sep = ",";
	}
	if (statemask & XT_CONNTRACK_STATE_DNAT) {
		printf(poolformat,"%sDNAT", sep);
		strcat(pfirerule->expand,poolformat);
		sep = ",";
	}
}
...
print_status(unsigned int statusmask)
{
	...
	if (statusmask & IPS_EXPECTED) {
		sprintf(poolformat,"%sEXPECTED", sep);
		strcat(pfirerule->expand,poolformat);
		sep = ",";
	}
	if (statusmask & IPS_SEEN_REPLY) {
		sprintf(poolformat,"%sSEEN_REPLY", sep);
		strcat(pfirerule->expand,poolformat);
		sep = ",";
	}
	if (statusmask & IPS_ASSURED) {
		sprintf(poolformat,"%sASSURED", sep);
		strcat(pfirerule->expand,poolformat);
		sep = ",";
	}
	if (statusmask & IPS_CONFIRMED) {
		sprintf(poolformat,"%sCONFIRMED", sep);
		strcat(pfirerule->expand,poolformat);
		sep = ",";
	}
	if (statusmask == 0)
		sprintf(poolformat,"%sNONE", sep);
		strcat(pfirerule->expand,poolformat);
}
                    ...
                    unsigned int family, bool numeric)
{
	...
	if (family == NFPROTO_IPV4) {
		if (!numeric && addr->ip == 0) {
			strcat(pfirerule->expand," anywhere");
			return;
		}
		if (numeric){
			sprintf(poolformat," %s%s",
           ...
           int inv, int numeric)
{
	...
	char poolformat[64];
	if (inv)
		strcat(pfirerule->expand," !");

	if (mask->s_addr == 0L && !numeric){
		sprintf(poolformat," %s", "anywhere");
		strcat(pfirerule->expand,poolformat);
	}
	else {
		if (numeric)
...
matchinfo_print(const void *ip, const struct xt_entry_match *match, int numeric, const char *optpfx)
{
	...
	if(sinfo->flags & XT_CONNTRACK_STATE) {
        	if (sinfo->invflags & XT_CONNTRACK_STATE)
				strcat(pfirerule->expand," !");
		sprintf(poolformat," %sctstate", optpfx);
		strcat(pfirerule->expand,poolformat);
	...
	if(sinfo->flags & XT_CONNTRACK_PROTO) {
        	if (sinfo->invflags & XT_CONNTRACK_PROTO)
			 strcat(pfirerule->expand," !");
		sprintf(poolformat," %sctproto", optpfx);
		strcat(pfirerule->expand,poolformat);
		sprintf(poolformat," %u", sinfo->tuple[IP_CT_DIR_ORIGINAL].dst.protonum);
		strcat(pfirerule->expand,poolformat);
	}
	...
	if(sinfo->flags & XT_CONNTRACK_ORIGSRC) {
		if (sinfo->invflags & XT_CONNTRACK_ORIGSRC)
			strcat(pfirerule->expand," !");
		sprintf(poolformat," %sctorigsrc", optpfx);
		strcat(pfirerule->expand,poolformat);
		print_addr(
	...
	if(sinfo->flags & XT_CONNTRACK_ORIGDST) {
		if (sinfo->invflags & XT_CONNTRACK_ORIGDST)
			strcat(pfirerule->expand," !");
		sprintf(poolformat," %sctorigdst", optpfx);
		strcat(pfirerule->expand,poolformat);
		print_addr(
	...
	if(sinfo->flags & XT_CONNTRACK_REPLSRC) {
		if (sinfo->invflags & XT_CONNTRACK_REPLSRC)
			strcat(pfirerule->expand," !");
		sprintf(poolformat," %sctreplsrc", optpfx);
		strcat(pfirerule->expand,poolformat);
		print_addr(
	...
	if(sinfo->flags & XT_CONNTRACK_REPLDST) {
		if (sinfo->invflags & XT_CONNTRACK_REPLDST)
			strcat(pfirerule->expand," !");
		sprintf(poolformat," %sctrepldst", optpfx);
		strcat(pfirerule->expand,poolformat);
		print_addr(
	...
	if(sinfo->flags & XT_CONNTRACK_STATUS) {
			...
			strcat(pfirerule->expand," !");
		sprintf(poolformat," %sctstatus", optpfx);
		strcat(pfirerule->expand,poolformat);
	...
	if(sinfo->flags & XT_CONNTRACK_EXPIRES) {
        	if (sinfo->invflags & XT_CONNTRACK_EXPIRES)
				strcat(pfirerule->expand," !");
		sprintf(poolformat," %sctexpire ", optpfx);
		strcat(pfirerule->expand,poolformat);
        	if (sinfo->expires_max == sinfo->expires_min){
				sprintf(poolformat,"%lu", sinfo->expires_min);
				strcat(pfirerule->expand,poolformat);
			}
        	else{
				sprintf(poolformat,"%lu:%lu", sinfo->expires_min, sinfo->expires_max);
				strcat(pfirerule->expand,poolformat);
			}
	}
	...
	if (sinfo->flags & XT_CONNTRACK_DIRECTION) {
		if (sinfo->invflags & XT_CONNTRACK_DIRECTION){
			sprintf(poolformat," %sctdir REPLY", optpfx);
			strcat(pfirerule->expand,poolformat);
		}
		else{
			sprintf(poolformat," %sctdir ORIGINAL", optpfx);
			strcat(pfirerule->expand,poolformat);	
		}
	}
		     ...
		     u_int16_t port_low, u_int16_t port_high)
{
	...
	if (port_high == 0 || port_low == port_high){
		sprintf(poolformat," %s%s %u", prefix, opt, port_low);
		strcat(pfirerule->expand,poolformat);
	}
	else{
		sprintf(poolformat," %s%s %u:%u", prefix, opt, port_low, port_high);
		strcat(pfirerule->expand,poolformat);
	}
}
               ...
               unsigned int family, bool numeric, bool v3)
{
	...
	if (info->match_flags & XT_CONNTRACK_PROTO) {
		if (info->invert_flags & XT_CONNTRACK_PROTO)
			strcat(pfirerule->expand," !");
		sprintf(poolformat," %sctproto %u", prefix, info->l4proto);
		strcat(pfirerule->expand,poolformat);
	}
	...
	if (info->match_flags & XT_CONNTRACK_ORIGSRC) {
		if (info->invert_flags & XT_CONNTRACK_ORIGSRC)
			strcat(pfirerule->expand," !");
		sprintf(poolformat," %sctorigsrc", prefix);
		strcat(pfirerule->expand,poolformat);
	...
	if (info->match_flags & XT_CONNTRACK_ORIGDST) {
		if (info->invert_flags & XT_CONNTRACK_ORIGDST)
			strcat(pfirerule->expand," !");
		sprintf(poolformat," %sctorigdst", prefix);
		strcat(pfirerule->expand,poolformat);
	...
	if (info->match_flags & XT_CONNTRACK_REPLSRC) {
		if (info->invert_flags & XT_CONNTRACK_REPLSRC)
			strcat(pfirerule->expand," !");
		sprintf(poolformat," %sctreplsrc", prefix);
		strcat(pfirerule->expand,poolformat);
	...
	if (info->match_flags & XT_CONNTRACK_REPLDST) {
		if (info->invert_flags & XT_CONNTRACK_REPLDST)
			strcat(pfirerule->expand," !");
		sprintf(poolformat," %sctrepldst", prefix);
		strcat(pfirerule->expand,poolformat);
	...
	if (info->match_flags & XT_CONNTRACK_STATUS) {
		if (info->invert_flags & XT_CONNTRACK_STATUS)
			strcat(pfirerule->expand," !");
		sprintf(poolformat," %sctstatus", prefix);
		strcat(pfirerule->expand,poolformat);
	...
	if (info->match_flags & XT_CONNTRACK_EXPIRES) {
		if (info->invert_flags & XT_CONNTRACK_EXPIRES)
			strcat(pfirerule->expand," !");
		sprintf(poolformat," %sctexpire ", prefix);
		strcat(pfirerule->expand,poolformat);

		if (info->expires_max == info->expires_min){
			sprintf(poolformat,"%u", (unsigned int)info->expires_min);
			strcat(pfirerule->expand,poolformat);
		}
		else{
			sprintf(poolformat,"%u:%u", (unsigned int)info->expires_min,
	...
	if (info->match_flags & XT_CONNTRACK_DIRECTION) {
		if (info->invert_flags & XT_CONNTRACK_DIRECTION){
			sprintf(poolformat," %sctdir REPLY", prefix);
			strcat(pfirerule->expand,poolformat);
			
		}
		else{
			sprintf(poolformat," %sctdir ORIGINAL", prefix);
			strcat(pfirerule->expand,poolformat);
		}
	}
}
