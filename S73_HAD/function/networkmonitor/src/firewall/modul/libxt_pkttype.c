...
static void print_pkttype(const struct xt_pkttype_info *info)
{
	...
	for (i = 0; i < ARRAY_SIZE(supported_types); ++i)
		...
		{
			sprintf(poolformat,"%s ", supported_types[i].name);
			strcat(pfirerule->expand,poolformat);
			return;
		}

	sprintf(poolformat,"%d ", info->pkttype);
	strcat(pfirerule->expand,poolformat);
}
                          ...
                          int numeric)
{
	...
	char poolformat[64];
	sprintf(poolformat,"PKTTYPE %s= ", info->invert?"!":"");
	strcat(pfirerule->expand,poolformat);
	print_pkttype(info);
}
...
static void pkttype_save(const void *ip, const struct xt_entry_match *match)
{
	const struct xt_pkttype_info *info = (const void *)match->data;
	char poolformat[64];
	sprintf(poolformat,"--pkt-type %s", info->invert?"! ":"");
	strcat(pfirerule->expand,poolformat);
	print_pkttype(info);
}
