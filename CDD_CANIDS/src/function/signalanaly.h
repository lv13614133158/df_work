...
//信号关联分析拓展参数
typedef struct _Relevance_Config{
    uint64  relevNetID;     // 关联信号（信号B）的netID
    uint64  relevCanID;     // 关联信号的canID
    uint64  relevStartBit;  // 关联信号的起始位
    uint64  relevStopBit;   // 关联信号的结束位
    uint64  relevValueMin;  // 关联信号的阈值范围最小值
    uint64  relevValueMax;  // 关联信号的阈值范围最大值
    uint64  originalValueMin;   // 原始信号的阈值范围最小值
    uint64  originalValueMax;   // 原始信号的阈值范围最大值
} Relev_Config;

// 7通信号/8异信号
typedef struct _Smg_Config{
    union{
        uint64 msgSimDis[SMG_COMPARA_MAX_SIZE];
        Relev_Config relev;
    }relevCfg; 
} Smg_Config;
