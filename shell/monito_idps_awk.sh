...
get_process_stats() {
    ...
    if [ -n "$PROCESS_pid" ]; then
        ...
        if echo "$cpu_line" | grep -q "Cpu(s):"; then
            ...
            # OpenWrt/BusyBox格式 - 根据您提供的输出，%CPU在第8列且没有百分号
            IDS_cpu=$(echo "$top_output" | grep " $PROCESS_pid " |grep -v 'grep'| awk '{print $8}' | grep -E '^[0-9]+\.?[0-9]*$' | tr -d ' \t' || echo "0")
        fi
