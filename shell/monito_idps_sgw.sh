...
get_system_stats() {
    ...
    # 获取第一行CPU信息: User 21%, System 18%, IOW 0%, IRQ 5%
    cpu_line=$(echo "$top_output" | head -1)
    
    # 检查是否是Android/嵌入式格式
    if echo "$cpu_line" | grep -q "User"; then
        # Android/嵌入式格式: User 21%, System 18%, IOW 0%, IRQ 5%
        usr_cpu=$(echo "$cpu_line" | sed -n 's/User \([0-9]*\)%.*/\1/p')
        sys_cpu=$(echo "$cpu_line" | sed -n 's/.*System \([0-9]*\)%.*/\1/p')
        
        # 如果解析失败，设置默认值
        if [ -z "$usr_cpu" ]; then usr_cpu="0"; fi
        if [ -z "$sys_cpu" ]; then sys_cpu="0"; fi
        
        # 计算空闲CPU (100 - User - System)
        idle_cpu=$((100 - usr_cpu - sys_cpu))
        if [ $idle_cpu -lt 0 ]; then idle_cpu=0; fi
    else
        # 判断top输出格式并相应解析
...
get_process_stats() {
    # 首先尝试从top输出中直接获取进程信息
    process_line=$(echo "$top_output" | grep "$PROCESS_NAME" | head -1)
    
    # 如果没找到匹配的进程名，尝试通过ps命令查找PID
    if [ -z "$process_line" ]; then
        # 使用ps命令查找进程PID
        PROCESS_pid=$(ps | grep "$PROCESS_NAME" | grep -v grep | grep -v sh | grep -v bash | awk '{print $1}' | head -1)
        
        # 如果找到了PID，从top输出中获取该PID的详细信息
        if [ -n "$PROCESS_pid" ] && echo "$PROCESS_pid" | grep -qE '^[0-9]+$'; then
            process_line=$(echo "$top_output" | grep "^$PROCESS_pid " | head -1)
        fi
    else
        # 从找到的行中提取PID
        PROCESS_pid=$(echo "$process_line" | awk '{print $1}')
    fi
    ...
    if [ -n "$PROCESS_pid" ] && echo "$PROCESS_pid" | grep -qE '^[0-9]+$'; then
        ...
        if echo "$cpu_line" | grep -q "User"; then
            ...
            if [ -z "$process_line" ]; then
                # 再次尝试获取进程行
                process_line=$(echo "$top_output" | grep "^$PROCESS_pid ")
            fi
            
            if [ -n "$process_line" ]; then
                # 提取CPU% (第4列)并去掉%符号
                raw_cpu=$(echo "$process_line" | awk '{print $4}')
                IDS_cpu=$(echo "$raw_cpu" | sed 's/%//' | grep -E '^[0-9]+\.?[0-9]*$' | tr -d ' \t')
                
                # 提取RSS内存 (第8列)并去掉K符号
                raw_mem=$(echo "$process_line" | awk '{print $8}')
                IDS_mem_RSS=$(echo "$raw_mem" | sed 's/K//' | grep -E '^[0-9]+$' | tr -d ' \t')
            else
                IDS_cpu="0"
                IDS_mem_RSS="0"
            fi
        elif echo "$cpu_line" | grep -q "Cpu(s):"; then
            # 标准Linux格式
...
print_stats() {
    ...
    if [ "$count_IDS_stats" -gt 0 ]; then
        ...
        avg_IDS_mem_RSS=$(echo "scale=0; $sum_IDS_mem_RSS / $count_IDS_stats" | bc 2>/dev/null | awk '{printf "%.0f", $0}' || echo "0")
...
# 捕获Ctrl+C信号以打印最终统计
trap 'echo ""; echo "Final stats:"; print_stats; exit 0' 2
