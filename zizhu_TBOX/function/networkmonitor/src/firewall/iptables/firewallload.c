...
static char commands_v_options[NUMBER_OF_CMD][12] =
{
/*INSERT*/    {'x',' ',' ',' ',' ',' ','x',' ',' ','x',' ',' '},
/*DELETE*/    {'x',' ',' ',' ',' ',' ','x',' ',' ','x','x',' '},
...
static int inverse_for_options[12] =
{
/* -n */ 0,
/* -s */ IPT_INV_SRCIP,
/* -d */ IPT_INV_DSTIP,
	       ...
	       struct xtc_handle *const handle)
{
	...
	if (!(format & FMT_NOTARGET))
		strcat(pfirerule->target,targname);

	strcat(pfirerule->prot,(fw->ip.invflags & IPT_INV_PROTO ? "!" : "")); 
	{
		const char *pname = proto_to_name(fw->ip.proto, format&FMT_NUMERIC);
		if (pname)
			strcat(pfirerule->prot,pname);
		else{
			sprintf(poolformat,"%hu", fw->ip.proto);
			strcat(pfirerule->prot,poolformat);
		}
	}

	if (format & FMT_OPTIONS) {
		if (format & FMT_NOTABLE)
			strcat(pfirerule->opt,"opt");
		strcat(pfirerule->opt,fw->ip.invflags & IPT_INV_FRAG ? "!": "-");
		strcat(pfirerule->opt,(flags & IPT_F_FRAG ? "f" : "-"));
	}
	...
	if (format & FMT_VIA) {
		...
		sprintf(pfirerule->devicein,"%s",iface);
		...
		sprintf(pfirerule->deviceout,"%s",iface);
	}

	strcat(pfirerule->source,(fw->ip.invflags & IPT_INV_SRCIP ? "!" : ""));
	if (fw->ip.smsk.s_addr == 0L && !(format & FMT_NUMERIC))
		strcat(pfirerule->source,"anywhere");
	else {
		if (format & FMT_NUMERIC)
			sprintf(buf, "%s", xtables_ipaddr_to_numeric(&fw->ip.src));
		else
	...
	strcat(pfirerule->destination,(fw->ip.invflags & IPT_INV_DSTIP ? "!" : ""));
	if (fw->ip.dmsk.s_addr == 0L && !(format & FMT_NUMERIC))
		strcat(pfirerule->destination,"anywhere");
	else {
		if (format & FMT_NUMERIC)
			sprintf(buf, "%s", xtables_ipaddr_to_numeric(&fw->ip.dst));
		else
			sprintf(buf, "%s", xtables_ipaddr_to_anyname(&fw->ip.dst));
		strcat(buf, xtables_ipmask_to_numeric(&fw->ip.dmsk));
		sprintf(poolformat,"%s", buf);
		strcat(pfirerule->destination,poolformat);
	}
	if(pfirerule->num  == 6)
		pfirerule->num = pfirerule->num ;
	...
	if (target) {
		...
		sprintf(poolformat,"[%lu bytes of unknown target data] ",t->u.target_size - sizeof(*t));
	     ...
	     int expanded, int linenumbers, struct xtc_handle *handle)
{
	...
	for (this = iptc_first_chain(handle);this;this = iptc_next_chain(handle)) {
		...
		const char *pol = iptc_get_policy(this, &counters, handle);
		if(pol){
			sprintf(policy,"%s", pol);
		}
		else{
			unsigned int refs;
			if (!iptc_get_references(&refs, this, handle)){
				memcpy(policy,"ERROR obtaining refs",strlen("ERROR obtaining refs"));
			}
			else{
				sprintf(policy," (%u references)\n", refs);
			}
		}
 
		if((i = iptc_first_rule(this, handle)) == NULL){
			/**list data malloc**/
			pfirerule = (struct firerule *)malloc(sizeof(struct firerule));		
			memset(pfirerule,0,sizeof(struct firerule));
			memcpy(pfirerule->Chain,this,strlen(this));
