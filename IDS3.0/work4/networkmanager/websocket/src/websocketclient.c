...
static void *ProcessMessageThread(void *args)
{
	...
	while(!interrupted)
	{
		...
		if(node)
		{
			...
			if(networkManager_callback!=NULL)
			{
...
void wbs_socketSend(long long lseqnumber, char *data, bool send_only_once)
{
...
int wbsClient_init(void)
{
	...
	int ca_chose = networkManagerObj.cert_eable;
	char caPath[128] = {0};
	char crtPath[128] = {0};
	char keyPath[128] = {0};
	strncpy(caPath,networkManagerObj.certsPath,128);
	strcat(caPath,_IDPS_CA);
	strncpy(crtPath,networkManagerObj.certsPath,128);
	strcat(crtPath,_IDPS_CRT);
	strncpy(keyPath,networkManagerObj.certsPath,128);
	...
	if(ca_chose == 1)
	{
			info.client_ssl_ca_mem = networkManagerObj.ca_mem;
            info.client_ssl_ca_mem_len = networkManagerObj.ca_mem_len;
            info.client_ssl_cert_mem = networkManagerObj.cert_mem;
            info.client_ssl_cert_mem_len = networkManagerObj.cert_mem_len;
            info.client_ssl_key_mem = networkManagerObj.key_mem;
            info.client_ssl_key_mem_len = networkManagerObj.key_mem_len;
	}else{
