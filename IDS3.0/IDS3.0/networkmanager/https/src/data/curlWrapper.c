...
static void setCurlComopt(CURL *curlget)
{
    ...
    if(networkManagerObj.cert_eable == 1 && 
        networkManagerObj.cert_mem && \
        networkManagerObj.key_mem && \
        networkManagerObj.ca_mem)
    {
        // 使用内存中的证书
        struct curl_blob cert_blob = {0};
        cert_blob.data = networkManagerObj.cert_mem;
        cert_blob.len = networkManagerObj.cert_mem_len;
        cert_blob.flags = CURL_BLOB_COPY;
        curl_easy_setopt(curlget, CURLOPT_SSLCERT_BLOB, &cert_blob);
        curl_easy_setopt(curlget, CURLOPT_SSLCERTTYPE, "PEM");

        struct curl_blob key_blob = {0};
        key_blob.data = networkManagerObj.key_mem;
        key_blob.len = networkManagerObj.key_mem_len;
        key_blob.flags = CURL_BLOB_COPY;
        curl_easy_setopt(curlget, CURLOPT_SSLKEY_BLOB, &key_blob);
        curl_easy_setopt(curlget, CURLOPT_SSLKEYTYPE, "PEM");

        struct curl_blob ca_blob = {0};
        ca_blob.data = networkManagerObj.ca_mem;
        ca_blob.len = networkManagerObj.ca_mem_len;
        ca_blob.flags = CURL_BLOB_COPY;
        curl_easy_setopt(curlget, CURLOPT_CAINFO_BLOB, &ca_blob);
    }else{
