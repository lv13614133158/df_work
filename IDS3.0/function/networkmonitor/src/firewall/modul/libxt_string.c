...
print_hex_string(const char *str, const unsigned short int len)
{
	...
	for (i=0; i < len; i++){
		sprintf(poolformat,"0%x", (unsigned char) str[i]);
...
print_string(const char *str, const unsigned short int len)
{
	...
	for (i=0; i < len; i++) {
		if (str[i] == '\"' || str[i] == '\\')
			strcat(pfirerule->expand,"\"\"");
		sprintf(poolformat,"%c", (unsigned char) str[i]);
		strcat(pfirerule->expand,poolformat);
	}
	strcat(pfirerule->expand,"\" ");
}
...
string_print(const void *ip, const struct xt_entry_match *match, int numeric)
{
	...
	if (is_hex_string(info->pattern, info->patlen)) {
		sprintf(poolformat,"STRING match %s", (invert) ? "!" : "");
		strcat(pfirerule->expand,poolformat);
	...
	sprintf(poolformat,"ALGO name %s ", info->algo);
	strcat(pfirerule->expand,poolformat);
	if (info->from_offset != 0){
		sprintf(poolformat,"FROM %u ", info->from_offset);
	    strcat(pfirerule->expand,poolformat);
	}
	if (info->to_offset != 0){
		sprintf(poolformat,"TO %u ", info->to_offset);
	    strcat(pfirerule->expand,poolformat);
	}
	if (revision > 0 && info->u.v1.flags & XT_STRING_FLAG_IGNORECASE)
		 strcat(pfirerule->expand," ICASE");
}
...
static void string_save(const void *ip, const struct xt_entry_match *match)
{
	...
	if (is_hex_string(info->pattern, info->patlen)) {
		sprintf(poolformat,"--hex-string %s", (invert) ? "! ": "");
	    strcat(pfirerule->expand,poolformat);
		print_hex_string(info->pattern, info->patlen);
	} else {
		sprintf(poolformat,"--string %s", (invert) ? "! ": "");
	    strcat(pfirerule->expand,poolformat);
		print_string(info->pattern, info->patlen);
	}
	sprintf(poolformat,"--algo %s ", info->algo);
	strcat(pfirerule->expand,poolformat);
	if (info->from_offset != 0){
		sprintf(poolformat,"--from %u ", info->from_offset);
		strcat(pfirerule->expand,poolformat);
