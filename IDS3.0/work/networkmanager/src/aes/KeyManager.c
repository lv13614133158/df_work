...
char *getSN(void* _curl)
{
	...
	if (networkFunctionEnabled())
	{
        printf("33333333333333333333333333333getSN\n");
	    getManageKeyFromNetWork(_curl);
	    if(keyManager.sn[0]!='\0')
	        return keyManager.sn;
	}
...
static void getManageKeyFromNetWork(void* _curl)
{
    ...
    strncpy(cUrl,"https://vsocidps-uat.dfiov.com.cn",128);
    strcat(cUrl,GET_MANAGE_KEY_URL);
    printf("curl %s\n",cUrl);
    char *body = NULL;
    // 计算所需内存大小
    size_t body_len = strlen("{\"channel_id\":\"T00001\",\"udid\":\"LQH02501170001\",\"equipment_type\":\"t-box\",\"os_type\":\"linux\"}") + 1;
    ...
    strcpy(body, "{\"channel_id\":\"T00001\",\"udid\":\"LQH02501170001\",\"equipment_type\":\"t-box\",\"os_type\":\"linux\"}");
...
static void getSessionKeyFromNetwork(void *_curl)
{
    ...
    printf(" v %d  %s %s \n","session key manager:",sessionKey);
