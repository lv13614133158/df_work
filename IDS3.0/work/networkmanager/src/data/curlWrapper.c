...
static void setCurlCApath(char *mqttpath)
{
    ...
    memcpy(caPath, getcaPath(), strlen(getcaPath()));
    memcpy(keyPath, getcaPath(), strlen(getcaPath()));
    memcpy(crtPath, getcaPath(), strlen(getcaPath()));
    memcpy(caInfo, getcaPath(), strlen(getcaPath()));
...
BaseResponse_t *startPostfile(char* header,const char* remotepath, const char* localpath,const char* filename)
{
    ...
    if (retcode == CURLE_OK){
	    ...
	    curl_mime_free(mime);
        printf("i %s  %s \n","networkmanager","upload file error\n"); 
        size_t len = strlen(errbuf);
        if(len > 0){
