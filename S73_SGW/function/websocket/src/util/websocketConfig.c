...
static void wbsSetManageKey(char* _iinput){
	...
	if(_iinput){
		...
		idpslog(0,"idps_websocket","websocket managerkey get success");
	}
}
...
static void wbsSetSessionKey(char* _iinput){
	...
	if(_iinput){
		...
		idpslog(0,"idps_websocket","websocket sessionkey get success");
	}
}
...
static void wbsSetSnnumber(char* _iinput){
	...
	if(_iinput){
		...
		idpslog(0,"idps_websocket","websocket getsnbumber success");
	}
}
...
static void wbsSetTokennumber(char* _iinput){
	...
	if(_iinput){
		...
		idpslog(0,"idps_websocket","websocket gettokenbumber success");
	}
}
...
bool wbsSetKeyInfo(){
	...
	{
		idpslog(0,"idps_websocket","managerkey && sessionkey && snnumber && tokennumber get success");
		m_readyKeyFlag = true;
	}else
	{
		idpslog(0,"idps_websocket","getKey failed");		
	}
