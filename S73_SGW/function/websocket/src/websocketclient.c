...
void wbsClient_localWebsocketSendEx(char* _input){
	...
	{
		/*clear list*/
		idpslog(1,"Send list", "listwebsockettx list overflow! clear!");
		while (listsize > 0)
		{
...
static void wbsClient_localWebsocketSend(long long lseqnumber, char *data, bool send_only_once)
{
	...
	if (json)
	{
		...
		if (print_data)
		{
			...
			{
				idpslog(1,"Not add Map", print_data);
				free(print_data);
				cJSON_Delete(json);
				free(data);
				return;
...
void wbsClient_sendIDPSErr()
{
	long long lseqnumber=0;
	char* s = wbsGetformIDPSErr(&lseqnumber);
	if (networkFunctionEnabled())
	{
		wbsClient_localWebsocketSend(lseqnumber, s, 0);
	}
...
void wbsClient_readLoopData()
{
	...
	{
		run_cnt++;
		if (run_cnt % ((1000 / sleep_time_ms) * retry_send_s) == 0)
		{
			wbsSetRetrySend();
		}
		...
		void *user, void *in, size_t len)
{
	...
	case LWS_CALLBACK_CLIENT_ESTABLISHED: // 第一次建立连接
		...
		idpslog(1,"idps_websocket","++++websocket connection established\n");
		break;

	case LWS_CALLBACK_CLIENT_CLOSED:
		s_wbs_connection_succ = false;
		idpslog(1,"idps_websocket","----websocket connection closed\n");
		wsisend = NULL;
		break;
