...
int initCert(void)
{
	...
	if (ret != 0)
	{
		log_e("ConfigParse", "secCryptoInit fail!");
	}
	...
	if (ret != 0)
	{
		log_e("ConfigParse", "secCryptoDataGetsize SecCrypto_DataID_SGW_ROOT_CERT fail!");
	}
	if (data_size > 0)
	{
		...
		if (root_cert_buff)
		{
			...
			if (ret != 0)
			{
				log_e("ConfigParse", "secCryptoDataGet SecCrypto_DataID_SGW_ROOT_CERT fail!");
			}
			else
			{
				if (s_root_cert)
	...
	{
		log_e("ConfigParse", "root cert data_size err!");
	}
	...
	if (ret != 0)
	{
		log_e("ConfigParse", "secCryptoDataGetsize SecCrypto_DataID_SGW_DEV_CERT fail!");
	}
	if (data_size > 0)
	{
		...
		if (dev_cert_buff)
		{
			...
			if (ret != 0)
			{
				log_e("ConfigParse", "secCryptoDataGet SecCrypto_DataID_SGW_DEV_CERT fail!");
			}
			else
			{
				if (s_dev_cert)
	...
	{
		log_e("ConfigParse", "dev certificate data_size err!");
	}
	...
	if (ret != 0)
	{
		log_e("ConfigParse", "secCryptoDataGetsize SecCrypto_DataID_SGW_DEV_KEY fail!");
	}
	if (data_size > 0)
	{
		...
		if (dev_key_buff)
		{
			...
			if (ret != 0)
			{
				log_e("ConfigParse", "secCryptoDataGet SecCrypto_DataID_SGW_DEV_KEY fail!");
			}
			else
			{
				if (s_dev_key)
	...
	{
		log_e("ConfigParse", "dev key data_size err!");
	}

	ret = secCryptoDeinit();
	if (ret != 0)
	{
		log_e("ConfigParse", "secCryptoDeinit fail!");
	}
}
