...
// 寻找报文的ID是否存在配置文件
static int Find_ListIndex(List_Elmt* listElmt, uint8 netID, uint32 canID, uint32 elmtCnt)
{
...
{
    int index = Find_ListIndex(listElmt, data.netID, data.canID, elmtCnt);
    ...
    if( !isFind )
    {
        ...
        sendListCheckEventMsg(data.netID, data.canID);
        //return isFind;
    }
    return isFind;
}
