...
bool SK_Signal_Threshold(Signal_Elmt* smgElmt, SK_Data_Stru smgData)
{
    ...
    if(temp > smgElmt->rule.thre.valueRange_Max)
    {
        ...
        signal_threshold_event_update(EVENT_LEVEL_NOPASS, SK_SMG_THRESHOLD_MAX_EVENT, 1, smgData.netID, smgData.canID,
                                      smgElmt->signal_name, 1, smgData.data, smgData.len);
        smgElmt->rule.thre.valueState = 2;
        sendSignalThresholdEventMsg(smgData.netID, smgData.canID, 1, temp, smgElmt->rule.thre.valueRange_Min, smgElmt->rule.thre.valueRange_Max, smgData.data);
        ret = false;
    }
...
bool SK_Signal_ChangeRate(Signal_Elmt* smgElmt, SK_Data_Stru smgData)
{
    ...
    if(smgElmt->rule.rate.valueRun)
    {
        ...
        if(rate > smgElmt->rule.rate.valueRange_Max)
        {
            ...
            signal_changeRate_event_update(EVENT_LEVEL_NOPASS, 0x8701, 1 ,smgData.netID, smgData.canID, smgElmt->signal_name,
                                           rate, 1, smgData.data, smgData.len);
            uint32 msgRate = rate * 100;
            sendSignalChangeRateEventMsg(smgData.netID, smgData.canID, msgRate, smgElmt->rule.rate.valueRange_Min * 100, smgElmt->rule.rate.valueRange_Max * 100, smgData.data);
            ret = false;
        }
	    ...
	    signal_changeRate_event_update(EVENT_LEVEL_NOPASS, 0x8702, 1 ,smgData.netID, smgData.canID, smgElmt->signal_name,
...
bool SK_Signal_Enumerate(Signal_Elmt* smgElmt, SK_Data_Stru smgData)
{
    ...
    if( pos < 0 ){
        ...
        signal_enumerate_event_update(EVENT_LEVEL_NOPASS, 0x8801, 1, smgData.netID, smgData.canID, smgElmt->signal_name,
                                       temp, smgData.data, smgData.len);
        sendSignalEnumEventMsg(smgData.netID, smgData.canID, temp, smgData.data);
        return false;
    }
...
bool SK_Signal_TrackeCnt(Signal_Elmt* smgElmt, SK_Data_Stru smgData)
{
    ...
    if( (temp - smgElmt->rule.step.valueRange_Min) % smgElmt->rule.step.valueSetp ){
        signal_trackeCnt_event_update(EVENT_LEVEL_NOPASS, 0x8902, 1, smgData.netID, smgData.canID, smgElmt->signal_name,
                                    temp, 0, smgData.data, smgData.len);
        sendSignalStateEventMsg(smgData.netID, smgData.canID, 1, temp, smgElmt->rule.step.valueLast, smgElmt->rule.step.valueRange_Min, smgElmt->rule.step.valueRange_Max,smgData.data);
        return ret;
    }
...
bool SK_Signal_StatIdent(Signal_Elmt* smgElmt, SK_Data_Stru smgData)
{
    ...
    if(smgElmt->rule.stat.valueRun)
    {
        ...
        if(!ret){
            sendSignalCountEventMsg(smgData.netID, smgData.canID, 1, temp, smgElmt->rule.stat.valueLast, smgElmt->rule.stat.valueRun, smgData.data);
        }
        else{
...
bool SK_Signal_PreCondit(Signal_Elmt* smgElmt, SK_Data_Stru smgData)
{
    ...
    if(pos1 >= 0 && SK_SmgSataus_Init(0, 0))
    {
        ...
        signal_relate_event_update(EVENT_LEVEL_NOPASS, 0x8D01, 1, smgData.netID, smgData.canID,
...
{
    uint64 originalValue = combineBytes64(smgData.data, smgElmt->startBit, smgElmt->stopBit);
    uint64 relevValue = combineBytes64(smgData.data, smgElmt->rule.same.relevCfg.relev.relevStartBit, smgElmt->rule.same.relevCfg.relev.relevStopBit);
    if (originalValue <= smgElmt->rule.same.relevCfg.relev.originalValueMax && originalValue >= smgElmt->rule.same.relevCfg.relev.originalValueMin)
    {
        if (relevValue > smgElmt->rule.same.relevCfg.relev.relevValueMax || relevValue < smgElmt->rule.same.relevCfg.relev.relevValueMin)
        {
            sendSignalRelevanceEventMsg(smgData.netID, smgData.canID, 1, originalValue, smgElmt->rule.same.relevCfg.relev.originalValueMin, smgElmt->rule.same.relevCfg.relev.originalValueMax, smgData.data, 
            smgData.canID, 1, relevValue, smgElmt->rule.same.relevCfg.relev.relevValueMin, smgElmt->rule.same.relevCfg.relev.relevValueMax, smgData.data);
        }
    }
...
bool SK_Signal_Differences(Signal_Elmt* smgElmt, SK_Data_Stru smgData)
{
    ...
    for(int i=0; i< smgElmt->ruleLen; i++)
    {
        if(smgData.data[smgElmt->startBit+i] != smgElmt->rule.diff.relevCfg.msgSimDis[i]){
            disCnt++;
        }
    }
...
// 信号分析配置参数填充
static void SignalDef_Init(Signal_Elmt* signAnaly, uint32 index, uint8 type, uint64* para, uint8 paraLen)
{
...
bool SK_SignalInit(Signal_Elmt* signAnaly, uint32 index, uint8 netID, uint32 canID, uint8 startBit, uint8 stopBit,
    uint8 type, uint64* para, uint8 paraLen)
{
	SignalCom_Init(signAnaly, index, netID, canID, startBit, stopBit);
	SignalDef_Init(signAnaly, index, type,  para,  paraLen);
	return true;
}
