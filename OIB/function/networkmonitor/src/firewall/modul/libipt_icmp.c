			   ...
			   int numeric)
{
	...
	if (!numeric) {
		...
		if (i != ARRAY_SIZE(icmp_codes)) {
			sprintf(poolformat,"%s%s ",invert ? "!" : "",icmp_codes[i].name);
	...
	sprintf(poolformat,"type %u", type);
	strcat(pfirerule->expand,poolformat);
	if (code_min == code_max){
		sprintf(poolformat," code %u ", code_min);
		strcat(pfirerule->expand,poolformat);
	}
	else if (code_min != 0 || code_max != 0xFF){
		sprintf(poolformat," codes %u-%u ", code_min, code_max);
		strcat(pfirerule->expand,poolformat);
	}
}
                       ...
                       int numeric)
{
	...
	{
		sprintf(poolformat,"Unknown invflags: 0x%X ",icmp->invflags & ~IPT_ICMP_INV);
		strcat(pfirerule->expand,poolformat);
	}
}
